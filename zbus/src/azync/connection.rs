...
impl<S> Drop for ConnectionInner<S> {
    fn drop(&mut self) {
        ...
        // exexecutor) to cancel the task. Dropping the task cancels it automatically anyway. The
        // only diference is the explicit cancellation waits for it to be canncelled/resolved.
        block_on(
...
impl Connection {
    ...
    /// responsible to continuesly [tick the executor]. Failure to do so will result in hangs.
    ///
    /// [tick the executor]: https://docs.rs/async-executor/1.4.1/async_executor/struct.Executor.html#method.tick
    ...
    /// scheduler][tcts], this method must be called before the connection
    /// is dropped, otherwise your thread will hang. On the other hand, calling this method is
    /// optional when using multi-threaded external executors, such as [tokio's multi-thread
    /// scheduler][tmts].
    ///
    /// # Panics
    ///
    /// This method panics if called more than once on a connection. This extends to all connection
    /// clones.
    ///
